...
const GameContent = (() => {
 ...
 const producers=[1,2,3].map(tier=>({id:'producer_drinkgen_'+tier,outputs:['ice','drink'].flatMap(type=>(tier===1?[1]:tier===2?[.8,.2]:[.4,.4,.2]).map((weight,i)=>({itemId:`item_${type}_${String(i+1).padStart(2,'0')}`,weight:weight/2}))),energyCost:0,cooldown:0,charges:null,unlockId:null}));
 ...
 const tasks=basicIds.map((id,i)=>({id,name:['木板与苔藓','招牌与遮阳棚','外墙','内墙','地板'][i],zoneId:'zone_restaurant',phase:'basic',coinCost:50,prerequisiteIds:i?[basicIds[i-1]]:[],rewards:[],unlockIds:[],effectType:'clean',mapAnchor:[[1980,2990],[1860,2650],[1030,2610],[1190,1900],[1370,2350]][i]}));
