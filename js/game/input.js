...
function bindPlaytestInput(session,feedback,message,busy,label,center){
 ...
 function startHold(drag){
  const generator=session.board.slots[drag.from];
  ...
  function repeat(){
   ...
   if(result.kind==='spawn')holdTimer=setTimeout(repeat,REPEAT_INTERVAL);
  }
  holdTimer=setTimeout(repeat,HOLD_DELAY);
 }
 ...
 document.addEventListener('pointerdown',e=>{if(e.isPrimary===false&&(session.drag||orderPan))cancel();},true);
