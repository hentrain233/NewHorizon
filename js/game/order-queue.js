...
class OrderQueue{
 ...
 makeRequest(){const config=this.content.orders,[min,max,limit]=OrderQueue.stage(this.difficulty(),config),count=this.roll(1,limit),band=max-min<=1?min:min+this.roll(0,max-min-1),cap=Math.min(max,band+1),chains=config.chainIds.map(id=>this.content.chains.find(c=>c.id===id)).filter(c=>this.isEligible(c.id));if(!chains.length)return null;const requirements=Array.from({length:count},()=>{const chain=chains[this.roll(0,chains.length-1)],eligible=this.content.items.filter(d=>d.chainId===chain.id&&d.tier>=band&&d.tier<=cap);if(!eligible.length)throw new Error('No eligible order tiers: '+chain.id);const item=eligible[this.roll(0,eligible.length-1)];return {type:item.type,level:item.tier};}),lucky=count>1&&this.random()<(count===2?0.1:0.3),pay=level=>{let center=(2**(level-1))*4*(1+.2*level)+10*level;if(level<=5)center*=1-.1*(6-level);const low=.9+.02*level,high=1+.02*level,span=high-low,roll=()=>low+this.random()*span,factor=level>=9||lucky?Math.max(roll(),roll()):roll();return Math.round(center*factor*.7);},prices=requirements.map(r=>pay(r.level));return {requirements,reward:prices.reduce((sum,price)=>sum+price,0)+(lucky?Math.max(...prices):0),lucky,xpReward:config.xp,stageCount:this.completedCount};}
