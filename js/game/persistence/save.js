...
function serializeGame(runtime){
 ...
 orders:runtime.orders.entries.filter(e=>e.phase==='waiting').map(e=>({id:e.id,customerId:runtime.content.customers.find(c=>c.portraitPrefix===e.type)?.id,variant:e.variant,requirements:e.requirements.map(r=>({...item(r),delivered:!!r.delivered})),reward:e.reward,rewards:e.rewards||[],generator:e.generator||null,xpReward:e.xpReward||0,stageCount:e.stageCount})),generatorChance:runtime.orders.generatorChance??.01};
}
...
function hydrateGameSave(runtime,raw,warn=console.warn){
 ...
 else{fresh.currencies.energy=runtime.energyRules.initial;fresh.energy={updatedAt:runtime.now()};}
 ...
 const seen=new Set(),ids=new Set();const orders=s.orders.flatMap(e=>{const customer=c.customers.find(x=>x.id===e.customerId);if(!customer||!runtime.orders.catalog[customer.portraitPrefix]?.includes(e.variant)){recover(e,'orders');return [];}if(seen.has(e.customerId)||ids.has(e.id))throw new Error('Duplicate order/customer');seen.add(e.customerId);ids.add(e.id);integer(e.id,'order ID');integer(e.reward,'order reward');integer(e.xpReward??0,'order XP');if(!Array.isArray(e.requirements)||!e.requirements.length)throw new Error('Invalid requirements');const requirements=e.requirements.map(r=>{const i=item(r,'requirements');return i&&{...i,delivered:!!r.delivered};});if(requirements.some(r=>!r)){recover(e,'orders');return [];}if(requirements.every(r=>r.delivered))throw new Error('Waiting order already fulfilled');const rewards=e.rewards==null?[]:e.rewards.map(r=>{if(r?.type!=='currency'||r.currency!=='energy'||!Number.isSafeInteger(r.amount)||r.amount<1)throw new Error('Invalid order energy');return {type:'currency',currency:'energy',amount:r.amount};});const generator=!e.generator?null:(e.generator.level===1&&(e.generator.type==='drinkgen'||e.generator.type==='fishnet')?{type:e.generator.type,level:1}:(()=>{throw new Error('Invalid order generator');})());return [{id:e.id,type:customer.portraitPrefix,variant:e.variant,requirements,reward:e.reward,rewards,generator,xpReward:e.xpReward||0,stageCount:e.stageCount||0,phase:'waiting',completedAt:null}];});
 ...
 runtime.state=fresh;runtime.board.slots=board;runtime.orders.entries=orders.map((e,i)=>({...e,slot:i}));runtime.orders.completedCount=count;runtime.orders.serial=Math.max(serial,...orders.map(e=>e.id),0);runtime.orders.generatorChance=Number.isFinite(s.generatorChance)?Math.max(.01,Math.min(1,s.generatorChance)):.01;runtime.orders.refillReadyAt=null;return s;
}
