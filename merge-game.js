...
class MergeTestBoard {
 ...
 generate(i){
  const generator=this.slots[i],producer=this.content.producers.find(p=>p.id===this.definition(generator)?.producerId);if(!producer||!this.isProducerAvailable(producer))return {kind:'none'};
  const free=this.empty();if(!free.length)return {kind:'full'};
 ...
 move(from,to){
  if(!Number.isInteger(from)||!Number.isInteger(to)||from<0||to<0||from>=63||to>=63||from===to||!this.slots[from])return {kind:'none'};
